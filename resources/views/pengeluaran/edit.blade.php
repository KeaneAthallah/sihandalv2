                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="rekening_id" class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                            ...
                            @foreach($rekenings->where('tipe', 'belanja')->merge($pengeluaran->rekening_id ? $rekenings->where('id', $pengeluaran->rekening_id) : collect()) as $rek)
                    ...
                    <div>
                        ...
                        <select name="kegiatan_id" class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                            ...
                            @foreach($kegiatans as $kegiatan)
                                <option value="{{ $kegiatan->id }}" {{ old('kegiatan_id', $pengeluaran->kegiatan_id) == $kegiatan->id ? 'selected' : '' }}>{{ $kegiatan->kode_kegiatan . ' - ' . $kegiatan->nama_kegiatan }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="3" placeholder="Tambahkan keterangan jika diperlukan..." class="input">{{ old('keterangan', $pengeluaran->keterangan) }}</textarea>
