<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Edit UPT" :breadcrumbs="['Data Master', 'UPT', 'Edit']">
            <x-slot name="actions">
    ...
    <div class="max-w-2xl">
        <form action="{{ route('upt.update', $upt) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <x-card title="Data UPT">
                <div class="space-y-4">
                    @if(auth()->user()->isAdmin())
                        <div>
                            ...
                            <select name="opd_id" id="opd_id" required class="mt-1 w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                                ...
                                @foreach($opds as $opd)
                                    <option value="{{ $opd->id }}" @selected(old('opd_id', $upt->opd_id) == $opd->id)>{{ $opd->nama }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <x-text-input type="text" name="kode" id="kode" class="mt-1" value="{{ old('kode', $upt->kode) }}" placeholder="Contoh: 001" required/>
                        <x-input-error :messages="$errors->get('kode')"/>
                    </div>

                    <div>
                        <x-input-label for="nama" value="Nama UPT" required/>
                        <x-text-input type="text" name="nama" id="nama" class="mt-1" value="{{ old('nama', $upt->nama) }}" placeholder="Masukkan nama UPT" required/>
