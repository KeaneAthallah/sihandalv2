<x-app-layout>
    ...
    <x-card>
            ...
            <table class="w-full text-sm min-w-[800px]">
                <thead>
                    <tr class="bg-slate-50/70">
                        ...
                        <th class="text-left px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Kode Rekening</th>
                        <th class="text-left px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Rekening</th>
                        <th class="text-left px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Tipe</th>
                        <th class="text-right px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Saldo</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($rekenings as $idx => $rek)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 bg-slate-100 text-slate-700 text-xs font-mono font-semibold rounded-lg">
                                    {{ $rek->kode }}
                                </span>
                            </td>
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4 font-semibold text-primary text-right whitespace-nowrap">
                                Rp {{ number_format($rek->saldo / 1000000000, 1, ',', '.') }} M
                            </td>
                            <td class="px-4 py-4">
        ...
        <div class="mt-4 pt-4 border-t border-slate-100">
            <p class="text-sm text-slate-500">Menampilkan {{ $rekenings->count() }} rekening</p>
        </div>
    </x-card>
