    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div class="p-6 border-b border-slate-100">
                <h2 class="text-lg font-semibold text-slate-800">Form Tambah Posisi Kas</h2>
            </div>
            <form action="{{ route('posisi-kas.store') }}" method="POST">
                @csrf
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="saldo_awal" id="saldo_awal" step="0.01" min="0" value="{{ old('saldo_awal') }}" placeholder="0" required class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                        @error('saldo_awal')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="penerimaan" id="penerimaan" step="0.01" min="0" value="{{ old('penerimaan') }}" placeholder="0" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                        @error('penerimaan')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="pengeluaran" id="pengeluaran" step="0.01" min="0" value="{{ old('pengeluaran') }}" placeholder="0" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                        @error('pengeluaran')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
