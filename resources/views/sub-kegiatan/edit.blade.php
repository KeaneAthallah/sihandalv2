<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Edit Sub Kegiatan" :breadcrumbs="['Program & Kegiatan', $kegiatan->nama_kegiatan, 'Edit Sub Kegiatan']">
            <x-slot name="actions">
    ...
    <div class="max-w-2xl">
        <form action="{{ route('sub-kegiatan.update', ['kegiatan' => $kegiatan, 'subKegiatan' => $subKegiatan]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <x-card title="Data Sub Kegiatan">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input type="text" name="kode_sub_kegiatan" id="kode_sub_kegiatan" class="mt-1" value="{{ old('kode_sub_kegiatan', $subKegiatan->kode_sub_kegiatan) }}" placeholder="Contoh: 1.01.01" required/>
                        <x-input-error :messages="$errors->get('kode_sub_kegiatan')"/>
                    </div>

                    <div>
                        <x-input-label for="nama_sub_kegiatan" value="Nama Sub Kegiatan" required/>
                        <x-text-input type="text" name="nama_sub_kegiatan" id="nama_sub_kegiatan" class="mt-1" value="{{ old('nama_sub_kegiatan', $subKegiatan->nama_sub_kegiatan) }}" placeholder="Masukkan nama sub kegiatan" required/>
                        <x-input-error :messages="$errors->get('nama_sub_kegiatan')"/>
                    </div>

                    <div>
                        <x-input-label for="pagu" value="Pagu" required/>
                        <x-text-input type="number" name="pagu" id="pagu" class="mt-1" value="{{ old('pagu', $subKegiatan->pagu) }}" placeholder="Masukkan pagu anggaran" step="0.01" min="0" required/>
