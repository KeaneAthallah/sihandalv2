<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Tambah Penerimaan" :breadcrumbs="['Penerimaan', 'Tambah']" />
    </x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div class="p-6 border-b border-slate-100">
                <h2 class="text-lg font-semibold text-slate-800">Form Tambah Penerimaan</h2>
            </div>
            <form action="{{ route('penerimaan.store') }}" method="POST">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="kode_sumber_dana" value="{{ old('kode_sumber_dana') }}" placeholder="Opsional" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_sumber_dana" value="{{ old('nama_sumber_dana') }}" placeholder="Opsional" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                    ...
                    <div>
                        ...
                        <input type="number" name="target" value="{{ old('target', '0') }}" placeholder="0" step="0.01" min="0" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" required>
                        @error('target')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="2" placeholder="Opsional" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
