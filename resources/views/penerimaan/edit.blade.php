            ...
            <form action="{{ route('penerimaan.update', $penerimaan) }}" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="sumber_dana_id" class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                            ...
                            @foreach($sumberDanas as $sd)
                                <option value="{{ $sd->id }}" {{ old('sumber_dana_id', $penerimaan->sumber_dana_id) == $sd->id ? 'selected' : '' }}>{{ $sd->nama_sumber_dana }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="3" placeholder="Tambahkan keterangan jika diperlukan..." class="input">{{ old('keterangan', $penerimaan->keterangan) }}</textarea>
