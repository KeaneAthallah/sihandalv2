<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <x-card title="Form Transfer Dana">
            <form action="{{ route('transfer-dana.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="jumlah" id="jumlah" min="0" value="{{ old('jumlah') }}" placeholder="0" required class="w-full pl-10 pr-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                        </div>
                ...
                <div>
                    ...
                    <input type="text" name="sumber_dana" id="sumber_dana" value="{{ old('sumber_dana') }}" placeholder="Masukkan sumber dana" required class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">
                    <x-input-error :messages="$errors->get('sumber_dana')" />
                </div>

                <div>
                    <x-input-label for="keterangan" value="Keterangan" />
                    <textarea name="keterangan" id="keterangan" rows="3" placeholder="Tulis keterangan jika diperlukan" class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition">{{ old('keterangan') }}</textarea>
                    <x-input-error :messages="$errors->get('keterangan')" />
                </div>

                <div class="mt-5 flex items-center justify-end gap-3">
                    <a href="{{ route('transfer-dana.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 hover:bg-slate-50 transition">
                        Batal
                    </a>
